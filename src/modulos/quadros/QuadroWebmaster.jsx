...
export default function QuadroWebmaster() {
  ...
  async function carregarServicos() {
    try {
      ...
      // Filtra só os serviços do turno 'webmaster'
      const somenteWebmaster = data.filter(
        (servico) => servico.turnoDaVez === "webmaster"
      );
      ...
      somenteWebmaster.forEach((servico) => {
      ...
      // Ordena pelo campo atualizadoEm para refletir a ordem correta
  ...
  async function onDragEnd(result) {
    ...
    if (destino === "concluido") {
      // Remove o turno atual "webmaster" das opções
    ...
    // Movimentação entre colunas normais
    try {
      await axios.put(
        `https://backend-gestao-paper.onrender.com/servicos/${itemMovido.id}`,
        {
  ...
  return (
    <div className="p-6">
      <h2 className="mb-4 text-2xl font-bold">Quadro Webmaster</h2>
