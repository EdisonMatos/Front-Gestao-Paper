...
export default function QuadroSuporte() {
  ...
  async function carregarServicos() {
    try {
      ...
      // **Filtro diferente: somente serviços do turno 'suporte'**
      const somenteSuporte = data.filter(
        (servico) => servico.turnoDaVez === "suporte"
      );
      ...
      somenteSuporte.forEach((servico) => {
  ...
  async function onDragEnd(result) {
    ...
    if (destino === "concluido") {
      // Aqui o turno atual é 'suporte', então removemos 'suporte' das opções
    ...
    // Mover entre colunas normais dentro do setor suporte
    try {
      await axios.put(
        `https://backend-gestao-paper.onrender.com/servicos/${itemMovido.id}`,
        {
  ...
  return (
    <div className="p-6">
      <h2 className="mb-4 text-2xl font-bold">Quadro Suporte</h2>
