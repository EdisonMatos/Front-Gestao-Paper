...
export default function QuadroDev() {
  ...
  // Função para carregar os serviços da API, organizar e ordenar por atualizadoEm
  async function carregarServicos() {
    try {
      ...
      const somenteDev = data.filter((servico) => servico.turnoDaVez === "dev");
      ...
      // Ordena cada coluna pelo campo atualizadoEm ascendente
  ...
  async function onDragEnd(result) {
    ...
    // Mover dentro da mesma coluna (ordenar verticalmente)
    if (origem === destino) {
    ...
    // Mover para "concluido" e perguntar setor
    ...
    // Mover entre colunas normais
    try {
      await axios.put(
        `https://backend-gestao-paper.onrender.com/servicos/${itemMovido.id}`,
        {
          posicaoNoQuadro: destino,
          atualizadoEm: new Date().toISOString(), // atualiza também para garantir ordem nova
        }
      );
